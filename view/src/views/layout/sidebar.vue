  ...
  <aside class="site-sidebar" :class="sidebarClasses">
    <div class="site-sidebar__inner">
      <el-menu
        ...
        <el-menu-item v-if="isAuth('sys:comm:home')" index="1-1" @click="$router.push({ name: 'home' })">
        ...
        return API.menu.nav().then(({data}) => {
          if (data && data.code === 0) {
            ...
            data.menuList.forEach(item => {
              ...
              if (item.type === 3) {
                appMenuList.push(item)
              }
            })
            sessionStorage.setItem('leftMenuList', JSON.stringify(leftMenuList || '[]'))
            sessionStorage.setItem('appMenuList', JSON.stringify(appMenuList || '[]'))
            this.UPDATE_MENU_NAV_LIST(leftMenuList)
            sessionStorage.setItem('permissions', JSON.stringify(data.permissions || '[]'))
            this.MyEvent.$emit('menu-init', 'menuInit')
          } else {
